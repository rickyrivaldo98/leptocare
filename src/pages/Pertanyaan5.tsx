...
const Pertanyaan5: React.FC<props> = ({ setForm, formData }) => {
            ...
            <IonPage className="relative">
                <div className="bg-green-400 h-1/3 text-center px-4">
                    <div className="flex pt-5">
                        <Link className="w-1/3 text-black text-xs text-left font-bold" to="/pertanyaan4">
                            <div>Kembali</div>
                        </Link>
                        <div className="w-1/3 text-black text-xs"><b>5</b> dari 11</div>
                        <div className="w-1/3"></div>
                    </div>
                    <div className="flex pt-12">
                        <div className="w-4/6">
                            <div className="text-left text-xl font-bold">Apakah Anda Mengalami Demam Lebih Dari 38 Derajat?</div>
                ...
                <div className="bg-white h-2/3 px-4">
                    <div className="pt-5 space-y-5">
                        <div onClick={() => { setActive(1); setNilai(1); setForm({ ...formData, pertanyaan5: 1 }) }} className={`relative rounded-md p-4 flex justify-between border ${active === 1 ? "border-green-700 bg-green-200" : "border-gray-300"} `}>
                        ...
                        <div onClick={() => { setActive(2); setNilai(2); setForm({ ...formData, pertanyaan5: 0 }) }} className={`relative rounded-md p-4 flex justify-between border ${active === 2 ? "border-red-700 bg-red-200" : "border-gray-300"} `}>
                            <label className="mr-3 cursor-pointer">
                    ...
                    <IonButton routerLink="/pertanyaan6" className="mb-2 absolute bottom-10 left-1/2 transform -translate-x-1/2" size="large"><div className="px-16 text-2xl">Selanjutnya</div></IonButton>
                </div>
            </IonPage>
        </>
    );
